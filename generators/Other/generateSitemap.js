...
const processNodes = (nodes, key) => {
	...
	nodes.forEach(({ node }) => {
		...
		const isVideo =
			context && context.mediaTypes && context.mediaTypes.default && context.mediaTypes.default === 'video';

		let versionUpdated = packageJson['version-updated-at'];
		const thumbnail =
			(context && context.normalized && context.normalized.image && context.normalized.image.src) ||
			(context && context.image && context.image.src);
