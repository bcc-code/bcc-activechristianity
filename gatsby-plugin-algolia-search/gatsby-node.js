...
async function getAlgoliaObjects(state, indexToUse, matchFields) {
  ...
  if (state._fetchingAlgoliaObjects) return state._fetchingAlgoliaObjects;
    ...
    state._fetchingAlgoliaObjects = fetchAlgoliaObjects(
      indexToUse,
      matchFields
    );
...
exports.onPostBuild = async function (
  ...
  if (typeof queries === 'function') {
    allQueries = await queries();
  }

  activity.report(`${allQueries.length} queries to index`);
  console.log(allQueries.length);
  const jobs = allQueries.map(async function doQuery(
    {
      indexName = mainIndexName,
      query,
    ...
    if (enablePartialUpdates) {
      ...
      const algoliaObjects = await getAlgoliaObjects(
        currentIndexState,
        indexToUse,
        matchFields
      );
      ...
      if (results) {
        hasChanged = objects.filter((curObj) => {
          ...
          const extObj = (currentIndexState.checked[ID] =
            currentIndexState.checked[ID] || algoliaObjects[ID]);
  ...
  try {
    ...
    if (enablePartialUpdates) {
      ...
      const cleanup = Object.keys(indexState).map(async function (indexName) {
        ...
        if (toRemove.length) {
          activity.report(
            `deleting ${toRemove.length} object from ${indexName} index`
          );
