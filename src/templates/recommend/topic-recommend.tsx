...
const TaxonomyPage: React.FC<ITaxonomyPageProps> = props => {
	...
	const { id, title, formats, breadcrumb, pagePath, latest, popular, featured } = pageContext;
	const path = pagePath;
	...
	return (
		<div>
			<MetaTag title={title} translatedUrls={[]} type="page" breadcrumb={[menusItems.topic]} path={path} />
				...
				{formats.length > 0 ? (
					<FetchTopicPostItems
						...
						render={({ topicPostItems }) => (
							<ScrollNavTabs
								tabs={topicPostItems.map(item => ({
									name: item.name,
									to: item.slug,
									content: (
										<div>
											{item.posts.slice(0, 6).map(p => {
												return <RightImgPostItem {...p} />;
											})}
