				...
				.then(res => {
					...
					setPreNextIds({
						previousId:
							getIndex < 1
								? parent
								: `${ac_strings.wallpaper_slug}/${quotes[getIndex - 1].id}?${paramString}&index=${
										getIndex - 1
								  }`,
						nextId:
							getIndex > 10
				...
				>
					...
					<div className="h-full w-full pt-10 overflow-scroll relative ">
						<div className="absolute bottom-1/2 right-0 left-0 z-10 text-white w-full flex p-2 justify-between">
							<Link className="shadow bg-white opacity-50 rounded-lg" to={preNextIds.previousId}>
								<KeyboardArrowLeftIcon className="text-ac-slate-dark" />
							</Link>
							<Link className="shadow bg-white opacity-50 rounded-lg" to={preNextIds.nextId}>
