...
const AllWallpapers: React.FC<IQuoteWallpaperProps> = ({ pageContext, path }) => {
	...
	return (
		<div className="relativeh-full pt-9 sm:pt-4 pb-4 standard-max-w-px">
			<MetaTag title={metaTitle || title} type="page" path={pagePath} breadcrumb={breadcrumb} />
			<PostH1 title={title} />
			<WallpaperModal
			...
			/>
			{isHomePage === true && <WallpaperFilter {...filterProps} />}
					...
					return (
						<LazyLoad key={id}>
							<StaggerChildrenItem>
								<Link
									to={`${
										ac_strings.wallpaper_slug
									}/${id}?openModal=true&parent=${pagePath}&index=${k}&filter=${!isHomePage}`}
