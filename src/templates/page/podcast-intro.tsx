	...
	return (
		<div className="max-w-sm mx-auto">
			<MetaTag
				...
				translatedUrls={[]}
				type="page"
				breadcrumb={[...breadcrumb, { name: introPost.title, to: path }]}
				path={path}
			/>
			<PodcastPageHeadSection>
				...
				<div className="pt-6 pb-12 flex items-center px-4">
					<div className="bg-ac-gray-light rounded-full inline-block">
						<ReadOrListenIcon listen={introPost.duration?.listen} track={introPost.media} />
					</div>
					<div className="text-xs text-white mx-4">{dateToString(introPost.date)}</div>
				</div>
			</PodcastPageHeadSection>
			<div className="py-6 sm:hidden">
				<PageSectionHeader title={ac_strings.meet_the_host} />
				<Tabs
					tabs={hosts.map(h => {
						return {
							header: {
			...
			<div className="hidden sm:block">
				<div className="max-w-sm px-4 py-6 mx-auto">
					<PageSectionHeader title={ac_strings.meet_the_host} className="pt-6" />
					<div>
						{hosts.map((h, i) => {
							return (
								<div
									className={`flex py-6 px-4 rounded-xl ${
										i % 2 == 0 ? 1 : 2 ? 'bg-ac-slate-lighter' : ''
									}`}
									key={i}
								>
									<div className="flex flex-col justify-center" style={{ order: i % 2 == 0 ? 1 : 2 }}>
										...
										<span className={`block text-roboto font-semibold text-center py-2`}>
											{h.name}
										</span>
									</div>
