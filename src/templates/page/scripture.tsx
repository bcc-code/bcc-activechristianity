...
const BibleNav: React.FC<IBibleNavProps> = props => {
	const {
		pageContext: { breadcrumb, title, bible },
		path
	} = props;
	...
	const result = (
		<div className="w-full rounded-lg  flex flex-wrap justify-start">
			{activeBook &&
				activeBook.availableChapters &&
				activeBook.availableChapters.map(chpt => {
					return chpt.a ? (
						<Link
						...
						<button className="w-8 text-center py-1 text-ac-gray font-light" key={chpt.v}>
							{chpt.v}
						</button>
					);
				})}
	...
	return (
		<ResourceLayout title={title}>
			<MetaTag title={title} type="page" path={path} translatedUrls={[]} breadcrumb={breadcrumb} />

			<div className="standard-max-w-px flex flex-wrap pb-24 flex-col sm:flex-row">
				<div className="w-full sm:w-1/2 pr-2 sm:pl-0">
					<div className="flex flex-wrap  ">
						<div className="flex w-full items-center justify-between pt-4 pb-2 text-lg border-b border-gray-300 mb-4 sticky top-0 bg-white">
							<h2 className="font-roboto">{ac_strings.old_testimony}</h2>
						</div>
						<PlaceHolder loading={bible.old.length === 0}>
							{bible &&
								bible.old.map((book, i) => {
									return (
										<div
											key={book.no}
