			...
			<div className="relative flex-1">
				...
				<div className="sm:hidden">
					<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 px-4 pb-4">
						{links.map(({ id, onClick, selected, name }) => (
							<div
								className="border rounded p-2 font-roboto font-semibold text-ac-slate-light"
								key={id}
								onClick={onClick}
								selected={selected}
							>
				...
				{sections.map((item, k) => {
					return (
						<Section id={item.slug}>
							<div className={`${k % 2 == 1 ? 'bg-ac-gray-light' : ''}`}>
								<div className="w-full">
									<CustomizedPageComponent title={item.title} items={item.childPage} />
								</div>
							</div>
						</Section>
				...
				<div
					className="hiddem sm:flex flex-col fixed bottom-0 right-0 mx-3 py-2 mb-16 bg-white shadow rounded-full text-white text-sm"
					style={{ zIndex: 60 }}
				>
