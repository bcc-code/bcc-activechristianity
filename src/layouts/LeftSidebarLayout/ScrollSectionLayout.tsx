...
const ScrollSectionLayout: React.FC<IProps> = ({ sections, title }) => {
			...
			<div className="pt-16">
				<SectionLink section={'about-us'}>
					{({ onClick, isSelected }) => (
						<div
							role="button"
							className={`tracking-wide text-base sm:text-lg sm:mb-2 leading-loose ${
			...
			<Section id={'about-us'} className={`pb-16 relative`}>
				...
				<div className="w-tablet px-4 pt-4">
					<p className="leading-loose italic">
						ActiveChristianity® by Brunstad Christian Church explores how God’s Word challenges and empowers
						us to live 100% according to His will, so we no longer need to fall in sin, but can come to a
						life of victory.
					</p>
				</div>
			</Section>
	...
	return (
		<ScrollingProvider offset={0}>
			<LeftSidebarLayout title={title} sidebar={ScrollSectionSidebar} content={content} />
		</ScrollingProvider>
	);
};
