...
export const PostLayout: React.FC<IPlaylist> = post => {
	...
	const share = (
		<div className="flex">
			...
			/>
			<div className="mx-4">
				<ToggleFollowPlaylistBookmark id={id} color="slate-light" />
			</div>
		</div>
	);
				...
				<FetchLatestPlaylists
					...
					render={({ playlists }) => {
						const randomPlaylist = getRandomArray(
							...
							playlists.length > 4 ? 4 : playlists.length
						);
						return (
	...
	return (
		<article className="overflow-scroll">
			...
			<MobilePostMain id={id} height={300} title={title} excerpt={excerpt} shareSlug={shareSlug}>
				{body}
			</MobilePostMain>
			<DesktopPostMain
				id={id}
				...
				headerMeta={<div className="flex justify-end text-ac-slate-dark">{share}</div>}
			>
				{body}
			</DesktopPostMain>
