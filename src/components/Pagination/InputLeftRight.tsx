...
const PostList: React.FC<IProps> = props => {
	...
	return (
		<div className="flex justify-center text-gray-500 font-roboto">
			<Link className="bg-ac-slate-lighter rounded  p-2 mr-2" to={getLinkPath(currentPage - 1)}>
				<KeyboardArrowLeftIcon />
			</Link>
			<form onSubmit={handleSubmit} className="flex border rounded py-1 overflow-hidden">
				<input
					type="text"
					className="w-8 text-center text-ac-slate-dark text-sm"
					value={pageInput}
			...
			<Link className="bg-ac-slate-lighter rounded  p-2" to={getLinkPath(currentPage + 1)}>
