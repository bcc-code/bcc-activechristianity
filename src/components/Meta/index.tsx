			...
			{({ navigate, location }) => (
				<div>
					<Helmet>
						...
						{tags && tags.length
							? tags.map((t: INavItem) => <meta property="article:tag" key={t.to} content={t.name} />)
							: null}
						...
						{authors
							? authors.map((a: INavItem, k: number) => (
									<meta
										key={k}
										property="book:author"
										content={`${process.env.SITE_URL}/${ac_strings.slug_ac_author}/${a.to}`}
									/>
							  ))
							: null}
						{imageUrl
