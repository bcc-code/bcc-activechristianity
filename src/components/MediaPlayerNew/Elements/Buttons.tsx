...
export const PlayButton: React.FC<IPlayPauseProps> = ({ border }) => {
	return (
		<PlayPauseWrapper border={border}>
			<polygon key="play" points="14,11 26,18 14,25" style={{ transformOrigin: '100% 50%' }} />
		</PlayPauseWrapper>
	);
};
...
export const PlayPauseControl: React.FC<IPlayPauseControlProps> = ({
	fullScreenInfo,
	handleClick,
	playPause,
	border
	...
	return (
		<div className={`flex items-center mx-4 ${fullScreenInfo ? 'py-2 sm:py-8' : ''}`}>
			<button onClick={handleClick}>
				{playPause ? <PauseButton border={border} /> : <PlayButton border={border} />}
			</button>
		</div>
	);
};
