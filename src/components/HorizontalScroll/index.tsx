...
export const PostlistHorizontalSimple: React.FC<{ posts: IPostItem[] }> = ({ posts }) => {
	return (
		<MotionStagger className="scroll-snap-x-container h-full">
			{posts.map(item => (
...
export const PostHorizontalScrollTypes: React.FC<IPostHorizontalScroll> = ({ posts, postThumnailType, postProps }) => {
	...
	return (
		<MotionStagger className="scroll-snap-x-container">
			{posts.map(post => (
				<div
					key={post.slug}
					className={`scroll-snap-x-child min-w-10/12 md:min-w-4/12 lg:min-w-3/12 ml-4 md:ml-0 md:pl-4`}
				>
...
export const HScrollBaseCustomSize: React.FC<IXScrollItem> = ({ items, childeClassName }) => {
	return (
		<div
			className="scroll-snap-x-container scroll-snap-x-container-start overflow-scroll mb-4 sm:hidden w-full"
			key={shortid()}
		>
			{items.map((c, i) => {
				return (
