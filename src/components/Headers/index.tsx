...
export const LayoutH1: React.FC<{ title: string | JSX.Element; icon?: JSX.Element; className?: string }> = ({
	title,
	icon,
	className
}) => (
	<h1 className={`font-semibold text-xl sm:text-2xl md:text-3xl py-6 flex items-center ${className}`}>
...
export const PostH1: React.FC<{ title: string }> = ({ title }) => (
	<h1
		className=" font-semibold text-ac-slate-dark text-3xl sm:text-4xl sm:leading-tight mb-2 sm:mb-6"
...
export const PageSectionHeaderUpperCaseGray: React.FC<{ title: string }> = ({ title }) => (
	<span className="block uppercase font-roboto font-normal text-gray-500 text-sm tracking-wider pb-2">{title}</span>
);
...
export const UnderlineTitleLink: React.FC<IUnderlineTitleLink> = ({ name, to }) => {
	...
	const text = (
		<span
			className="block text-sm sm:text-base border-b border-ac-primary pb-1 absolute uppercase font-roboto "
			style={{ bottom: '-1px' }}
		>
			{name}
		</span>
	...
	if (to) {
		...
		return <div className={mainClass}>{text}</div>;
	}
};
