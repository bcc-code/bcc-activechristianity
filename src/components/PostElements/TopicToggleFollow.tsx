...
import {
	...
	BookmarkBorderIcon,
	LocalOfferIcon
} from '@/components/Icons/MUI/bookmarkIcons';
	...
	return (
		<FetchAndSetFollowedTopics
			...
			render={({ followed }) => {
				...
				return (
					<button>
						{followed === 'loading' && <CachedIcon customSize="4" key={shortid()} />}
						{followed === 'true' && <CheckIcon customSize="4" key={shortid()} />}
						{followed === 'false' && <AddIcon customSize="4" key={shortid()} />}
					</button>
				);
			}}
		/>
		...
		<FetchAndSetFollowedPlaylists
			...
			render={({ followed }) => {
				return (
					<div>
						{followed === 'loading' && (
							<CachedIcon customSize={buttonSize} className={`fill-${buttonColor}`} />
						)}
						{followed === 'false' && (
							<BookmarkBorderIcon customSize={buttonSize} className={`fill-${buttonColor}`} />
						)}
						{followed === 'true' && (
	...
	return (
		<FetchAndSetFollowedTopics
			...
			render={({ followed }) => {
				...
				return (
					<div
						className={`flex justify-center py-1 px-2 my-2 w-full text-center text-sm rounded-full font-semibold ${config.color}`}
					>
	...
	return (
		<FetchAndSetFollowedTopics
			...
			render={({ followed }) => {
				...
				return (
					<div
						className={`flex justify-center py-1 px-2 my-2 w-full text-center text-xs rounded-full font-semibold ${config.color}`}
					>
						<span>{text ? text : config.text}</span>
					</div>
				);
...
export const ToggleFollowWithName: React.FC<ITopicNavItem> = ({ id, name, to }) => {
	return (
		<div
			className={`flex py-2 px-4 mb-2 mr-2 text-center text-sm rounded-sm items-center bg-gray-200 text-gray-500`}
		>
			<Link to={to} className="pr-2">
				{name}
			</Link>
			<FetchAndSetFollowedTopics
				id={id}
...
export const TopicWithIcon: React.FC<ITopicNavItem> = ({ name, to }) => {
	return (
		<Link to={`${to}`} className="border border-ac-slate-light rounded-lg px-1 flex items-center mr-2 mb-2 py-1">
			<LocalOfferIcon customSize="2" className="fill-slate-light" />
			<span className="px-1 text-ac-slate-light" style={{ fontSize: '12px' }}>
				{name}
			</span>
		</Link>
	);
