...
export const ReadMore: React.FC<{ slug: string; id: string }> = ({ slug, id }) => {
	return (
		<div className="flex justify-between">
			<Link className="block text-indigo-500 text-sm" to={slug}>
				{ac_strings.read_more}
			</Link>
...
export const AuthorLink: React.FC<{ authorGroups?: IPostAuthors[] }> = ({ authorGroups }) => {
	return (
		<span>
			{authorGroups && authorGroups[0] && authorGroups[0].authors
				? authorGroups[0].authors.map((item, k) => (
						<Link
							key={k}
							className="inline-block post-meta-commar"
							to={`${ac_strings.slug_ac_author}/${item.to}`}
						>
							{item.name}
						</Link>
				  ))
...
export const ReadingTimingIcon: React.FC<{ read?: string; listen?: string }> = ({ read, listen }) => {
	return (
		<span className={'mr-6 flex items-center'}>
			<AccessTimeIcon className="fill-slate-dark" customSize="5" />
			<span className="text-xs sm:text-sm text-ac-slate-dark pl-2 whitespace-no-wrap">{read}</span>
		</span>
	);
};
...
export const ReadIcon: React.FC<{ text?: string }> = ({ text }) => (
	<div className={'flex items-center'}>
		<DescriptionIcon className="fill-slate-dark" customSize="5" />
		...
		<div className="">
			{playing ? (
				<div className="flex items-center bg-ac-slate-dark  rounded-full py-1 px-2">
					<EqualizerIcon customSize="5" className="fill-slate-light" />
					<span className="text-xs sm:text-sm text-white pl-2 whitespace-no-wrap">{ac_strings.playing}</span>
				</div>
			) : (
				<div className="flex items-center rounded-full bg-ac-slate-lighter py-1 px-2">
					<PlayCircleOutlineIcon className="fill-slate-dark" customSize="5" />
...
export const ReadOrListenIcon: React.FC<{ read?: string; listen?: string; track?: IMedia }> = ({
	read,
	listen,
	track
}) => {
		...
		<span>
			{listen && track ? (
				<FetchAndSetCurrentMedia
					...
					clickable
					render={({ playing }) => {
						return <ListenIcon playing={playing} text={listen} />;
					}}
				/>
			) : (
...
export const Views: React.FC<{ views: string }> = ({ views }) => {
	return (
		<div className="mx-2 flex items-center">
			<VisibilityIcon className="fill-slate-dark" customSize="5" />
			<span className="text-xs sm:text-sm text-ac-slate-dark pl-2 whitespace-no-wrap">{views}</span>
		</div>
	);
};
