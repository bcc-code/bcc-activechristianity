...
export default class Headroom extends Component<IProps, IState> {
  ...
  update = () => {
    ...
    if (!this.isOutOfBound(this.currentScrollY)) {
      ...
      if (this.state.state === 'unfixed') {
        if (action === 'unpinned' || action === 'none')
          this.setState({ unfixedToUnpinned: true });
      } else {
        this.setState({ unfixedToUnpinned: false });
      }
  ...
  render() {
    ...
    let innerStyle: any = {
      ...
      background: this.props.disable || this.state.state === 'unfixed' ? 'linear-gradient(white 60%, rgba(255, 255, 255, 0.75))' : '#fff',
      top: 0,
      left: 0,
      right: 0,
      zIndex: 1,
    ...
    if (this.state.unfixedToUnpinned === false && this.state.state !== 'unfixed') {
      ('adding transtion')
      innerStyle = {
        ...innerStyle,
