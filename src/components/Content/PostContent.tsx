...
const TextSelectPopper: React.FC<{
	className?: string;
	content: string;
	glossary: IGlossary[];
	title: string;
	slug: string;
}> = ({ children, className, slug, title, glossary, content }) => {
			...
			if (isGlossary || hasSelected) {
				if (selectionInfo) {
					if (contentEl && contentEl.current) {
						...
						if (isForward) {
							toSetPostion.right =
								contentEl.current.clientWidth - position.right + contentDivPosition.left;
						} else {
	...
	return (
		<article ref={contentEl} onMouseUp={handleMouseUp} className={`relative ${className ? className : ''}`}>
			{showPopper === true && (
				<div ref={popperEl} style={{ position: 'absolute', ...position, height: '0px' }}>
					<ShareIconPopper shareUrl={options.shareUrl} text={shareText} title={title} />
				</div>
			)}
			{showGlossary && (
				<div
				...
				>
					{/* {g.content}... */}
					<GlossaryPopper text={glossaryContent?.content} slug={glossaryContent?.slug} />
