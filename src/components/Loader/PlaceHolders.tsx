...
export const RowPlaceholder: React.FC<{ loading: boolean }> = ({ loading, children }) => {
	const customerPlaceholder = (
		<div className="standard-max-w px-4 grid sm:grid-cols-2 md:grid-cols-4 gap-4 pb-16 w-full">
			{['', '', '', ''].map((item, k) => {
				return <TopImgPlaceholder key={k} />;
			})}
		</div>
	);
	return (
	...
	return (
		<div className="z-10 grid grid-cols-8 gap-12 standard-max-w-px relative pt-8 sm:pt-16 md:pt-20 lg:pt-24">
			<div className="col-start-1 col-end-6 ">
				<RectShape
					color="WhiteSmoke"
					className="text-gray-300 w-full sm:my-10 min-h-48 sm:min-h-64 md:min-h-64"
				/>
			</div>
...
export const RightImgListPlaceHolder: React.FC<{ count: number }> = ({ count }) => {
	...
	return (
		<div className="px-4">
			{arr.map((item, i) => {
				return <RightImgPlaceholder key={i} />;
			})}
		</div>
	);
};
...
export const ScriptureBookBlock = () => {
	return <RectShape color="WhiteSmoke" className="text-gray-300 w-full min-h-8" />;
};
