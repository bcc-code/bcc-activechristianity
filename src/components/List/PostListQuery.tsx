	...
	React.useEffect(() => {
		...
		if (currentPage !== 1) {
			acApiModule.then(res => {
				...
				if (id && subTopicId) {
					api.getPostsPerPageQueryBySubtopicId(id, subTopicId, currentPage).then(res => {
					...
					api.getPostsPerPageQueryByTopicId(id, currentPage).then(res => {
						if (res && res.topic && res.topic.somePosts && Array.isArray(res.topic.somePosts.data)) {
							const receivedPosts = res.topic.somePosts.data.map(item => normalizePostRes(item));
	...
	return (
		<div className="max-w-sm">
			...
			{loading ? (
				...
				<MotionStagger>
					{posts.map((p, k) => {
						return <RightImgWDes key={shortid()} {...p} />;
					})}
				</MotionStagger>
			)}
