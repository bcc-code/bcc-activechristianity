...
const FeaturedCard: React.FC<IFeaturedCard> = ({
	...
	return (
		<PostListItemMotion
			className={`${
				className ? className : ''
			} relative justify-end select-none block rounded-xxl sm:rounded-xl border border-gray-300 hover:shadow-md p-2 md:p-4 h-full w-full text-white overflow-hidden`}
		>
			<div
				className="absolute inset-0 rounded-xxl sm:rounded-xl overflow-hidden bg-center bg-cover"
				style={bgStyle}
			></div>
			<div
				className=" absolute left-0 top-0 bottom-0 right-0 rounded-lg"
			...
			<div className="flex flex-col rounded-lg h-full relative">
				<Link to={postSlug} className="w-full flex items-center justify-center flex-1 p-2 md:p-4 z-50">
					{(type === 'podcast' || type === 'playlist') && (
						<div className="relative w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32">
							<div
								id="play-button"
								className="absolute p-2 text-white z-10 inset-0 flex justify-center items-center"
							>
				...
				<Link to={postSlug} className="flex flex-col">
					<div className="flex flex-col justify-center sm:justify-end z-30 h-12 sm:h-15 md:h-18">
						{type == 'playlist' && (
							<span className="text-xs text-white text-center py-2">{ac_strings.playlist} </span>
						)}
						<PostTitle
							rawText={title}
							bold={'font-semibold'}
					...
					<div
						...
						} z-30 w-full border-b border-gray-500 my-2 md:my-4`}
					/>
				</Link>
				<div
					className={`${showOnMobile ? 'block' : 'hidden sm:flex'} justify-between items-center text-sm z-30`}
				>
					{authors && authors[0] && (
						<div className="">
					...
					{type === 'playlist' && <ToggleFollowPlaylistBookmark id={id} color="white" size="6" />}
				</div>
			</div>
		</PostListItemMotion>
	);
