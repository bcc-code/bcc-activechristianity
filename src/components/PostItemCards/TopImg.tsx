...
const TopImgPost: React.FC<IPostItem & ITopImgPost> = props => {
	const { title, slug, image, noBorder, noExcerpt, media, showType, format, noBg, fixedImageHeight } = props;
	return (
		<motion.div
			variants={{
			...
			className={`sm:hover:shadow-md flex flex-col max-w-lg text-gray-800 h-full overflow-hidden ${
				noBg === true ? 'bg-none' : 'bg-white '
			} ${noBorder !== true ? ' rounded-xxl sm:rounded-lg border ' : ''} `}
		>
			<PostItemMediaImg
				className={`relative w-full sm:pl-0 flex justify-end overflow-hidden ${
					noBorder === true ? 'rounded-lg' : 'rounded-lg rounded-b-none'
				}`}
				track={media}
				slug={props.slug}
			>
				...
				{fixedImageHeight ? (
					<LazysizesImage
						{...image}
						className={`w-full pointer-events-none object-cover g-image ${
							noBorder === true ? 'rounded-lg' : 'rounded-lg rounded-b-none'
						}`}
						alt={title}
					...
					<Image2To1
						image={image}
						imageClassName={` h-full pointer-events-none w-auto object-cover g-image ${
							noBorder === true ? 'rounded-lg' : 'rounded-lg rounded-b-none'
						}`}
						alt={title}
					/>
