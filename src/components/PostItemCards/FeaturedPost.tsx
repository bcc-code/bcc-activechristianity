		...
		<MotionAppear className="w-full z-10 grid grid-cols-1 md:grid-cols-8 md:gap-6 lg:gap-12 relative sm:pt-16 px-4">
			...
			<div className="md:col-start-6 md:col-end-9 md:ml-5 flex flex-col justify-center pt-0">
				<div className="flex flex-col justify-center pt-0">
					<Link to={`/${slug}`}>
						{format && format[0] && <PostLabel text={format[0].name} />}
						<TextSizeWClamp rawText={title} fontKey="header-post" clamp={3} className="sm:py-3 py-1" />
					</Link>
					...
					<div className="flex items-center sm:py-3 py-1">
						...
						{reading_time && (
							<p className="ml-1 text-sm text-ac-gray-dark lg:hidden">{reading_time.text}</p>
						)}
						<ReadingTimingAuthor
							className="w-full text-sm text-ac-gray-dark hidden lg:block"
