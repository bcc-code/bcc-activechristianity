	...
	return (
		<div className="text-sm flex lg:py-2 border-b border-gray-300 last:border-b-1 py-2 sm:py-4">
			<div className="flex-1 mr-4">
				...
				{authors && !small && (
					<div className="text-xs text-ac-slate-dark sm:text-ac-slate-light mb-2">
						{' '}
						<AuthorLink authorGroups={authors} />
					</div>
				)}
				{media.audio && <SimpleSmallListenIcon media={media} />}
			</div>
			<div className="flex items-center">
				<Link to={slug} className="h-16 w-16">
					<SquareImage className="rounded-lg" {...image} />
				</Link>
			</div>
		</div>
	);
