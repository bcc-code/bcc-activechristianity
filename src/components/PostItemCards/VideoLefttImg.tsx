...
const VideoPost: React.FC<IVideoPost> = ({
	slug,
	image,
	title,
	excerpt,
	...
	return (
		<Link to={`${slug}`} className={`flex sm:pr-2 h-full mt-2`}>
			...
			<div className="flex-1">
				...
				{showCategory && types && (
					<div className="pb-2">
						<span className="rounded uppercase p-1 text-mini bg-gray-200 text-gray-600 ">
							{types.map(cat => cat.name).join(' ')}
						</span>
					</div>
				)}
