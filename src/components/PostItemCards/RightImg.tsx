...
const RightImgWDes: React.FC<IRightImgNoDes> = props => {
	const { title, image, format, excerpt, border, slug, id, reading_time, authors, media, audioDuration, duration } =
		props;
	...
	return (
		<PostListItemMotion className={`${border !== false ? 'border-b border-gray-300 last:border-b-1 pb-4' : ''}`}>
			<div className="flex items-start sm:items-stretch w-full cursor-pointer mt-5">
				<div className={`pr-4 flex flex-col w-8/12 sm:w-6/12 md:w-8/12 items-start `}>
					...
					{authors && (
						<div className="text-xs text-ac-slate-dark sm:text-ac-slate-light mb-4">
							{' '}
			...
			<div className="pt-4 flex w-full justify-between">
				<ReadOrListenIcon track={media} {...duration} />
				<div className="flex">
