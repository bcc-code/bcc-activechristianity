...
export const OutlineScriptureChapter: React.FC<{ active?: boolean } & IButton> = ({ children, active }) => (
	<Button
		className={` whitespace-no-wrap text-sm border border-ac-gray hover:border-ac-slate-dark rounded-lg py-2 px-2 font-semibold ${
			active === true ? 'bg-ac-slate-dark text-white' : 'text-ac-slate-dark bg-white'
		}`}
	>
		{children}
	</Button>
...
export const UnderlineLink: React.FC<{ to: string }> = ({ to, children }) => {
	return (
		<Button to={to} className="text-ac-slate-dark underline text-sm font-semibold">
			{children}
		</Button>
	);
};
...
export const UnderlineLinkViewAll: React.FC<{ to: string }> = ({ to, children }) => {
	return (
		<Button to={to} className="text-ac-slate-dark underline text-sm">
			{ac_strings.see_all}
		</Button>
	);
};
...
export const OutlineButton: React.FC<IButtonName> = ({ name, disabled, onClick, href, to, className }) => {
	return (
		<Button
			...
			href={href}
			onClick={onClick}
			className={`border rounded-full flex justify-between items-center text-base sm:text-lg px-4 py-2 font-semibold ${
				className ? className : ''
			}`}
...
export const OutlineRightIcon: React.FC<IOutlineRightIcon> = ({ to, name, count, arrow }) => {
	return (
		<Button
			to={to}
			className={`${
				count ? 'border-ac-secondary text-ac-secondary' : ''
			} px-4 py-4  sm:px-6 border rounded-xxl sm:rounded-xl flex justify-between items-center`}
		>
			<div className="text-base sm:text-lg ">{name}</div>
			<div className="text-xs sm:text:sm">
				{count ? count : ''}
				{arrow && <KeyboardArrowRightIcon customSize="6" />}
			</div>
		</Button>
	);
};

export const SolidDarkBgToggleActive: React.FC<{ active?: boolean; className?: string } & IButton> = ({
	children,
	active,
	className,
	onClick
	...
	<Button
		className={`flex justify-center py-1 px-2 my-2 text-center text-sm rounded-full font-semibold whitespace-nowrap min-w-12 ${
			className ? className : ''
		} ${active === true ? 'bg-ac-slate-dark text-white' : 'bg-ac-slate-lighter text-ac-slate-dark'}`}
		onClick={onClick}
	>
		{children}
	</Button>
...
export const SideNavItem: React.FC<{ hideOnMobile?: boolean; hideOnDeskop?: boolean; next?: boolean } & IButton> = ({
	onClick,
	children,
	to,
	href,
	...
	return (
		<Button
			className={`w-full py-4 font-roboto font-semibold flex justify-center relative ${
				hideOnDeskop ? ' sm:hidden' : ''
			} ${hideOnMobile ? ' hidden sm:flex' : ''} ${className ? ` ${className}` : ''}`}
			onClick={onClick}
			to={to}
			href={href}
		>
