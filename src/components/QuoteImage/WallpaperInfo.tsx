...
const ShowWallpaperRelatedInfo: React.FC<IQuote> = wallpaper => {
	...
	return (
		<div className="pt-10">
			...
			{!isBibleQuote && wallpaper.author && (
				<h3 className="pb-4 italic">
					{ac_strings.by} {wallpaper.author.name}
				</h3>
			)}
			{!isAcArticle && wallpaper.source && (
				<h3 className="pb-4 italic">
					{ac_strings.from} {wallpaper.source}
				</h3>
			)}
			...
			{wallpaper.author && !isBibleQuote && (
				<>
					<h3 className="font-semibold pb-4">
						{ac_strings.more_from}{' '}
						<Link
							className="text-ac-secondary"
							to={`${ac_strings.slug_ac_author}/${wallpaper.author.slug}`}
						>
					...
					<FetchAuthorPosts
						...
						render={({ posts, loading }) => {
							const post = posts[0];
							return post ? <ColorBlockNoImg {...post} /> : <div></div>;
						}}
					/>
				</>
			)}
			{!hasNotBibleAuthor && wallpaper.topics && wallpaper.topics[0] && (
				<>
					<FetchTopicPostItems
						layout="list"
						topics={[wallpaper.topics[0]].map(t => ({
							...t,
							slug: `${ac_strings.slug_topic}/${t.slug}/1`
						}))}
								...
								<>
									{topicPostItems.map(t => {
										return (
											<>
												<h3 className="font-semibold pb-4">
													{ac_strings.posts_from_same_topic}{' '}
													<Link
														className="text-ac-secondary"
														to={`${ac_strings.slug_topic}/${t.slug}`}
													>
														{t.name}
													</Link>
												</h3>
			...
			{wallpaper.topics && (
				<div className="flex flex-wrap border-ac-gray py-6">
					{wallpaper.topics &&
						wallpaper.topics.map(t => {
							return (
								<ToggleFollowWithName
									key={t.slug}
									to={`${ac_strings.slug_topic}/${t.slug}/1`}
									name={t.name}
									id={t.id}
								/>
							);
