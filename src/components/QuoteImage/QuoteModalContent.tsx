...
const WallpaperModalContent: React.FC<ISwipeViewContent & { isActive?: boolean; background?: string }> = ({
	image,
	color,
	size,
	wallpaper,
	...
	return (
		<div className="pb-8 sm:w-3/4 md:w-tablet mx-auto md:grid md:grid-cols-2">
				...
				{findImage && isActive && (
					<Wallpaper
						className={`rounded-t-lg rounded-b-none md:rounded-l-lg md:rounded-r-none ${
							border ? 'border-t border-l border-b' : ''
						}`}
						key={findImage.id}
						image={findImage}
						size={size}
			...
			<div
				className={`py-6 px-4 bg-white rounded-b-lg md:rounded-r-lg md:rounded-l-none ${
					...
					<div>
						<div className="flex justify-between flex-col md:flex-row">
							...
							<div className="flex items-center md:order-2">
								<a
									className="w-full rounded-lg bg-ac-slate-dark text-white text-lg px-4 py-2 font-semibold text-center"
									href={findImage && findImage.src}
									onClick={e => download(e, findImage && findImage.id ? findImage.id : 1)}
								>
									{ac_strings.download}
								</a>
							</div>
