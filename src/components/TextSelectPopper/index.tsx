...
const TextSelectPopper: React.FC<{
	...
	title: string;
	slug: string;
}> = ({ children, className, slug, title, glossary }) => {
	...
	return (
		<article ref={contentEl} onMouseUp={handleMouseUp} className={`relative ${className ? className : ''}`}>
			{showPopper === true && (
				<div ref={popperEl} style={{ position: 'absolute', ...position, background: 'red', height: '0px' }}>
					<ToolTipShare shareUrl={options.shareUrl} text={shareText} title={title} />
				</div>
			)}
			{showGlossary && (
				<span className="text-sm leading-normal font-normal font-sans max-w-64 py-6">
					{/* {g.content}... */}
					Glossary
					<a target="_blank" href={`${ac_strings.slug_glossary}/`} rel="noreferrer">
						{' '}
