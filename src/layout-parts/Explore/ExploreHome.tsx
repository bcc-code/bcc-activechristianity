...
const ExploreLayout: React.FC<{
	...
	return (
		<div className="bg-white max-w-tablet mx-auto pb-8">
			...
			{process.env.LANG_CODE === 'en' && (
				<>
					<div className="pt-6 text-sm ">
						<SectionTitleDesktopAndMobile name={edification.name} to={edification.slug} />
						<FetchPostsFromSlugs
							layout={'list'}
					...
					<div className="pt-6">
						<SectionTitleDesktopAndMobile name={questions.name} to={questions.slug} />
						<FetchPostsFromSlugs
							layout={'list'}
							...
							render={({ posts }) => {
								return (
									<div className="px-4">
										{posts.map(p => {
											return <TitleLink key={p.slug} title={p.title} slug={p.slug} />;
										})}
									</div>
								);
							}}
			...
			{hasScriptureSection && <ExplorePopularScripture isMobile={isMobile} />}

			{process.env.LANG_CODE !== 'en' && (
				<div className="pt-6">
					<SectionTitleDesktopAndMobile name={ac_strings.recommend_for_you} />
			...
			{process.env.LANG_CODE === 'en' && (
				<div className="pt-6">
					<SectionTitleDesktopAndMobile name={ac_strings.banner_ebook_title} />
