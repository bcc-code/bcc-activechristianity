...
const ByTaxonomy: React.FC<IByTaxonomyProps & any> = props => {
	...
	const { items: topics, refine, currentRefinement, setTaxonomyFilter, sortTopicsMap } = props;
	...
	return (
		<div className="relative">
			<div className="flex items-end sm:items-center justify-end w-full">
				...
				<div
					className="flex items-center justify-end px-4 sm:pt-0"
					onClick={() => {
				...
				>
					...
					<p className="text-sm text-ac-secondary p-4">
						<i>
							{topics.length} {ac_strings.filters}
						</i>
			...
			{showFilter && (
				<div className="bg-ac-slate-lighter absolute max-w-tablet left-0 right-0" style={{ zIndex: 60 }}>
					{[
						...
						return (
							<div className={`px-4 pt-0`} key={item.title}>
								<h6 className="mb-2 pr-2 pt-4 font-bold text-sm sm:text-base">{item.title}</h6>
								<div className="flex flex-wrap">{item.list}</div>
							</div>
						);
					})}
...
const RefinementLabel: React.FC<{
	...
	return (
		<button
			className={`py-1 px-2 mr-2 my-1  ${
				isRefined ? `${color} text-white` : 'bg-white'
