...
const Filter: React.FC<any> = ({ byTopics, byFeaturedAuthors, byColors, slug }) => {
	...
	return (
		<>
			...
			>
				<button className="p-4 absolute right-0 top-0" onClick={() => setToggleFilter(false)}>
					<CloseIcon />
				</button>
				...
				<div className="flex flex-wrap">
					{byColors.map(c => {
						...
						return (
							<Link
								to={`${slug}/${c.name}`}
								className="block h-8 w-8 min-h-8 max-h-8 min-w-8 max-w-8 mr-4 mb-2 rounded-lg overflow-hidden"
								style={{ backgroundColor: rgbaColor, color: rgbaColor }}
							>
								{c.name}
							</Link>
						);
				...
				<div className="flex flex-wrap">
					{byFeaturedAuthors.map(t => {
						...
						return (
							<Link
								to={`${slug}/${t.slug}`}
								className={`mr-2 py-1 px-2 my-1 text-xs  bg-white flex justify-between items-center rounded-lg bg-ac-slate-lighter whitespace-nowrap`}
							>
								<span>{t.name}</span>
					...
					<div className="grid grid-cols-2 gap-2 ">
						{byFeaturedAuthors.slice(0, 3).map(t => {
							return (
								<Link
									key={t.slug}
									className="text-sm font-medium border rounded-lg p-2 flex items-center justify-center text-center"
									to={`${slug}/${t.slug}`}
								>
									{t.slug === 'the-bible' ? 'Bible verse wallpapers' : `${t.name}`}
								</Link>
							);
						})}
