...
export const MobileHeaderBackground: React.FC<{ imgUrl: string }> = ({ imgUrl, children }) => {
	return (
		<div
			className="sm:hidden fixed inset-x top-0 w-full flex items-center"
			style={{ top: '96px', height: '330px' }}
		>
			<div
		...
		return (
			<div className="border-ac-gray border-t py-8 text-gray-600">
				...
				<div className="flex flex-wrap text-sm pt-4">
					{translatedUrls.map((item, i) => (
						<a className="w-1/2 sm:w-1/3 md:w-1/4 pb-2" href={item.to} key={i}>
							{item.name}
						</a>
					))}
				</div>
		...
		<div className="flex flex-col mb-4">
			...
			<span className="">
				{authors.authors.map((a, i) => (
					<Link className="block text-sm pt-1" key={i} to={`${ac_strings.slug_ac_author}/${a.to}`}>
	...
	return (
		<div>
			<div className="border-ac-gray border-b pt-6">
				{authors?.map(item => {
					return <AuthorFollowSection key={shortid()} authors={item} />;
				})}
			</div>
							...
							<FetchPostsFromArchivePage
								...
								render={({ posts }) => {
									...
									return fourPosts.length > 0 ? (
										<Row3ColAndXScroll title={`${ac_strings.more_from} ${a.name}`} posts={posts} />
									) : (
										<div></div>
									);
								}}
...
export const AuthorBookmarkShareSection: React.FC<
	IShareLikesViewsProps & { authors?: IPostAuthors[]; formats?: ITopic[] }
> = props => {
	...
	return (
		<div className="relative bg-white border-ac-gray flex justify-between">
			...
			<div className="flex">
				<BookmarksAndViews id={id} views={views} likes={likes} />
				<ShareButton
					shareUrl={shareSlug}
					text={text ? text : ''}
...
export const ShareSection: React.FC<IShareLikesViewsProps> = props => {
	...
	return (
		<div className="relative bg-white border-ac-gray flex justify-between">
			<BookmarksAndViews id={id} views={views} likes={likes} />
			<ShareButton shareUrl={shareSlug} text={text ? text : ''} label={ac_strings.share} size="5" />
		</div>
	);
};

export const ShareBookmarkTopShortCuts: React.FC<IShareLikesViewsProps & { isPlayingAudio: boolean }> = ({
	id,
	shareSlug,
	text,
	isPlayingAudio
	...
	return (
		...
		>
			<button className="px-2 py-1" key={shortid()}>
				<ToogleBookmark id={id} />
			</button>
			<button className="px-2 py-1" key={shortid()}>
				<ShareButton shareUrl={shareSlug} text={text ? text : ''} color="secondary" size="6" placement="left" />
			</button>
			<button key={shortid()} className="px-2 py-1" onClick={scrollToTop}>
				<PublishIcon className="fill-secondary" customSize="6" />
			</button>
		</div>
	);
};
export const MobileMainWrapper: React.FC<{ height: number }> = ({ height, children }) => {
	return (
		<div className="sm:hidden">
			<div
				className="relative w-full h-full bg-white rounded-t-2xl mt-64 pt-4 px-4 z-50"
...
export const MobilePostMain: React.FC<IMobilePostMain> = ({ height, title, excerpt, children, translatedUrls }) => {
	return (
		<div className="sm:hidden">
...
export const DesktopPostMain: React.FC<IDesktopPostMain> = ({
	id,
	title,
	excerpt,
	headerLeft,
...
export const MoreLatestLink: React.FC<{ latestSlug: string }> = ({ latestSlug }) => (
	<div className="w-full flex justify-center py-4">
		<Link className="flex items-center px-2 py-1 text-ac-secondary text-sm" to={`/${latestSlug}`}>
