...
const ReadNext: React.FC<IFetchPost> = ({ topics, formats, postId, position, isPlayingAudio, slug }) => {
	...
	const deskTopicSlider = (
		<Flickity options={{ autoPlay: true, prevNextButtons: false, pageDots: false }}>
			...
			{preference.length > 0 && (
				<div className=" w-full py-4" key={shortid()}>
					<div className="font-roboto text-lg"></div>
			...
			<div className=" w-full py-4" key={shortid()}>
				<div className="font-roboto text-lg">You might be insterested in</div>
	...
	return (
		<div
			className={`fixed xl:relative transition-all duration-1000 flex flex-col bg-ac-slate-lighter veiw-next-wrapper ${
				showViewMore ? 'veiw-next-show' : 'veiw-next-hidden'
			}`}
		>
			...
			{!isPlayingAudio && topics && formats && (
				<div className="">
					{seriesId ? (
						<div
							ref={viewNextEl}
							className={`hidden lg:flex lg:absolute px-4 flex-col`}
							style={{ ...fixedStyle, left: 0 }}
						>
							<div className="sticky w-full flex flex-col" style={{ top: '150px', height: '260px' }}>
								{seriesId && <SeriesIndex title={getSeries.title} posts={getSeries.posts} />}
					...
					<div className="xl:hidden">
						<Flickity options={{ autoPlay: true, prevNextButtons: false, pageDots: false }}>
							...
							{topicPostItems.map(topic => {
								...
								return <TopicWithPost topic={topic} post={showPost} />;
							})}
...
const SeriesIndex = (data: { title: string; posts: { title: string; slug: string }[] }) => {
	...
	return (
		<div className="flex flex-col">
			<div className="block uppercase font-roboto text-gray-500 text-xs tracking-wider pb-2">Series</div>
			<div className="text-lg pb-6">{title}</div>
			{posts.map(item => {
				return (
		...
		<div className="text-sm flex">
			...
			<div className="flex-1 p-2">
				<Link to={post.slug} className="text-sm">
					<TextSizeTitle rawText={post.title} clamp={2} fontKey="text-sm" />
				</Link>
			</div>
		</div>
	);
	...
	return (
		<div className="w-full ">
			<Link to={topic.slug} className="h-20 w-20">
				{topic.image && <SquareImage {...topic.image} />}
			</Link>
			<div className="p-2">
		...
		<div className="w-full" key={shortid()}>
			<div className="text-sm flex">
				...
				<div className="flex-1 p-2">
					<Link
						to={topic.slug}
						className="block uppercase font-roboto text-gray-500 text-xs tracking-wider pb-2"
					>
