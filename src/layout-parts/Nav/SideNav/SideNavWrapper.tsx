...
const SideMobile: React.FC<ISideMobile> = ({ isSideNavOpen, close, back, title, children, className }) => {
    return (
        <div
            className={`drawer-side drawer-side-${isSideNavOpen ? 'open' : 'close'} drawer-side-submenu bg-d4slate-lighter w-full h-full flex flex-col justify-between xs:w-mobile xs:left-auto xs:shadow overflow-y-scroll fixed top-0 right-0 bottom-0 z-50 `}
        >
