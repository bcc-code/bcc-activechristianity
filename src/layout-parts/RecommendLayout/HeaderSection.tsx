	...
	return (
		<div className="relative hidden sm:flex mb-8">
			<div className="flex-1 pr-4">
				<Link to={`${headerPost.slug}`} className="relative z-30 flex flex-col justify-between text-sm">
					{videoUrl ? (
			...
			<div
				...
				style={{ width: '350px', minWidth: '350px', minHeight: '350px' }}
			>
				{video ? (
					listPosts.map(item => (
						<div>
					...
					<PopularPosts title={listTitle ? listTitle : ac_strings.popular} posts={listPosts} />
				)}
			</div>
		</div>
	);
