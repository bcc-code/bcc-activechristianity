...
const Preview = () => {
	...
	const handleSubmitPreivewForm = () => {
		const path = `/preview?id=${inputId}&type=${selectedType.value}`;
		navigate(path);
	};
			...
			api.getOnePreviewPostById(id)
				.then(res => {
					if (res && res.previewPost) {
						...
						const { id, langs, content, meta, recommendPosts, readMorePosts, seo, updated_at } =
							res.previewPost;
	...
	return !loading ? (
		<main className="">
			...
			{page && (
				<div>
					...
					<CustomizedPageComponent
						items={page.customizedPageComponents}
						slug={page.slug}
						title={page.title}
					/>
					<div className="w-full flex justify-center py-6">
						<Link
							to={`${ac_strings.slug_topic}/${page.slug}`}
			...
			{!post && !page && (
				<div className="max-w-tablet mx-auto px-4">
					...
					{errorMessage && <Snackbar text={errorMessage} error />}

					<div className="text-lg flex px-4 flex-col">
						<div style={{ position: 'relative', zIndex: 0 }}>
							<InputText label="Id" value={inputId} onChange={handleInputId} />
						</div>
						<div className="">
						...
						<FormSubmitButton className="my-6" onClick={handleSubmitPreivewForm} />
					</div>
				</div>
			)}
		</main>
		...
		<div className="max-w-tablet mx-auto">
			<h1
				...
				style={{
					fontSize: '80px',
					textShadow: '-1px -1px 0 var(--secondary), 0 0 1px var(--secondary), 4px 4px 0 var(--secondary)'
				}}
			>
				Loading
			</h1>
